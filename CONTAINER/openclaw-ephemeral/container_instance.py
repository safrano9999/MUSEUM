...
def mode(path):
    ...
    if value.isdigit() and 2 <= int(value) <= 5:
        return int(value)
    raise SystemExit(f"Invalid CONTAINER_NR={value!r} in {path}; use 2-5 or blank")
...
def main():
    ...
    used_numbers = {
        value for value in modes.values() if isinstance(value, int)
    }
    available_numbers = [
        number for number in range(2, 6) if number not in used_numbers
    ]
